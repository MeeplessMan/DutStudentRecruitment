...
public class EmployerMainActivity extends AppCompatActivity {
    ...
    private static String TAG = "EmployerMainActivity";
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        documentReference = db.collection("employer").document(user.getUid());
        loadUserProfile();
        setupAccountOptionsMenu();

        if (savedInstanceState == null) { // Load initial fragment only once
            getSupportFragmentManager().beginTransaction()
                    .replace(R.id.frameLayout, new EmployerMainFragment()) // Use the correct FrameLayout ID
        ...
        tabLayout.setOnTabSelectedListener(new TabLayout.OnTabSelectedListener() {
            ...
            public void onTabSelected(TabLayout.Tab tab) {
                ...
                switch (tab.getPosition()) {
                    case 0:
                        fragment = new EmployerMainFragment();
                        break;
                    case 1:
                        fragment = new EmployerCandidateSearchFragment();
                        break;
                    case 2:
                        fragment = new EmployerChatFragment();
                        break;
                    case 3:
                        fragment = new EmployerOurOffersFragment();
                        break;
                }
                if (fragment != null) {
    ...
    private void promptForPasswordBeforeDeletion() {
        ...
        builder.setPositiveButton("Verify", (dialog, which) -> {
            ...
            if (password.isEmpty()) {
                Toast.makeText(EmployerMainActivity.this, "Password cannot be empty.", Toast.LENGTH_SHORT).show();
                return;
            }
        ...
        user.reauthenticate(credential)
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful()) {
                        ...
                        Toast.makeText(EmployerMainActivity.this, "Incorrect password. Please try again.", Toast.LENGTH_LONG).show();
                    }
                });
    }
    ...
    private void deleteUserAccount() {
        ...
        user.delete()
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful()) {
                        Toast.makeText(EmployerMainActivity.this, "Account deleted successfully.", Toast.LENGTH_SHORT).show();
                        navigateToLogin();
                    } else {
                        Toast.makeText(EmployerMainActivity.this, "Failed to delete account: " + task.getException().getMessage() + ". Please try logging in again and then deleting.", Toast.LENGTH_LONG).show();
                    }
                });
    }
    private void logoutUser() {
        ...
        Intent intent = new Intent(EmployerMainActivity.this, LoginActivity.class); // Replace LoginActivity
        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
        startActivity(intent);
        finish();
    }
            ...
            auth.sendPasswordResetEmail(user.getEmail())
                    .addOnCompleteListener(new OnCompleteListener<Void>() {
                        ...
                        public void onComplete(@NonNull Task<Void> task) {
                            if (task.isSuccessful()) {
                                Toast.makeText(EmployerMainActivity.this, "Password reset email sent to " + user.getEmail(), Toast.LENGTH_LONG).show();
                            } else {
                                Toast.makeText(EmployerMainActivity.this, "Failed to send reset email: " + task.getException().getMessage(), Toast.LENGTH_LONG).show();
                            }
                        }
                    });
        } else {
            ...
            documentReference.get().addOnCompleteListener(task -> {
                if (task.isSuccessful()) {
                    ...
                    if (document != null && document.exists()) {
                        ...
                        if (imgProfile != null) {
                            if (profileImageUrl != null && !profileImageUrl.isEmpty()) {
                                Glide.with(EmployerMainActivity.this)
                    ...
                    if (getApplicationContext() != null) { // Check context before showing Toast
                        Toast.makeText(EmployerMainActivity.this, "Error loading profile.", Toast.LENGTH_SHORT).show();
                    }
